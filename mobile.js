...
(function(){
  ...
  `;
  if(!masters.length){
    const emptyTeam=root.querySelector('#tn13Team');
    if(emptyTeam) emptyTeam.hidden=true;
    root.querySelectorAll('a[href="#tn13Team"]').forEach(a=>a.remove());
  }
  const initialBookOptions=root.querySelectorAll('#tn13BookSheet .tn50-book-option');
  if(initialBookOptions[0]){
    initialBookOptions[0].href='tel:+37477116819';
    initialBookOptions[0].removeAttribute('aria-disabled');
  }
  if(initialBookOptions[1]){
    initialBookOptions[1].href='https://t.me/Boshki_projectt';
    initialBookOptions[1].target='_blank';
    initialBookOptions[1].rel='noopener';
    initialBookOptions[1].removeAttribute('aria-disabled');
    const label=initialBookOptions[1].querySelector('strong');
    if(label) label.textContent='Telegram';
  }
